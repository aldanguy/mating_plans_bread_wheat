...
titre_genetic_variance=${r_gblup}genetic_variance_${ID}.txt

param_renumf90_1=${r_gblup_param}renumf90_${ID}.par
log_renumf90_1=${r_log}renumf90_${ID}.out
param_airemlf90=${r_gblup_param}renf90_${ID}.par
log_airemlf90=${r_log}airemlf90_${ID}.out
param_renumf90_2=${r_gblup_param}postGSf90_1_${ID}.par
log_renumf90_2=${r_log}postgsf90_1_${ID}.out
param_postGSf90=${r_gblup_param}postGSf90_2_${ID}.par
log_postGSf90=${r_log}postGSf90_2_${ID}.out
log_predf90=${r_log}predf90_${ID}.out
...
then
    ...
    if [ ${qtls_info} == "ESTIMATED" ] && [ ${population} == "unselected" ]
    then


    titre_tbv_to_convert_to_phenotypes=${r_parents}TBV_first_generation_${ID}.txt

    titre_phenotypes_used=${r_parents}phenotypes_first_generation_${ID}.txt
    ...
    titre_phenotypes_used=${r_parents}phenotypes_last_generation_${ID}.txt
        titre_markers_used_blupf90=${titre_markers_qtls}
...
then
    ...
    titre_phenotypes_used=${titre_phenotypes_parents0}
...
then
    ...
    then
     ...
     echo "STEP 1.3 : convert genotypes to haplotypes"
        ...
        titre_genotyping_input=${titre_genotypes_used}
        titre_markers_input=${titre_markers0}
        titre_haplotypes_output=${titre_haplotypes_used}
        ...
        echo "STEP 1.4 : prepare_for_LDAK.R "


        titre_genotypes_parents_input=${titre_genotypes_used}
        titre_markers_input=${titre_markers0}
        titre_markers_output=${r_LDAK}LDAK_${ID}.map
        titre_genotyping_output=${r_LDAK}LDAK_${ID}.ped
        ...
        echo "STEP 1.5 : buld LDAK matrix "



        plink --file ${r_LDAK}LDAK_${ID} --recode --noweb --out ${r_LDAK}LDAK_2_${ID}  
        plink --file ${r_LDAK}LDAK_2_${ID} --noweb --make-bed --out ${r_LDAK}LDAK_3_${ID} 
        ...
        mkdir -p ${r_LDAK_temp}
        cd ${r_LDAK_temp}
        cp ${r_LDAK}LDAK_3_${ID}* ${r_LDAK_temp}


        /work/adanguy/ldak5.1.linux --cut-weights sections --bfile ${r_LDAK_temp}LDAK_3_${ID} --window-cm 1 --section-cm 1 --buffer-cm 1 --no-thin YES > ${r_log}LDAK_step1_${ID}.out
        /work/adanguy/ldak5.1.linux --calc-weights-all sections --bfile ${r_LDAK_temp}LDAK_3_${ID} > ${r_log}LDAK_step2_${ID}.out
        /work/adanguy/ldak5.1.linux --calc-kins-direct LDAK-Thin --bfile ${r_LDAK_temp}LDAK_3_${ID} --weights ${r_LDAK_temp}sections/weights.all --power -1 --kinship-raw YES > ${r_log}LDAK_step3_${ID}.out

        echo "STEP 1.6 : convert_LDAK_matrix.R"


        titre_g_raw=${r_LDAK_temp}LDAK-Thin.grm.raw
        titre_ibs_output=${titre_LDAK_used}
        titre_lines=${r_parents}TBV_last_generation_${ID}.txt
    ...
    then
        ...
        echo "STEP 1.x simulate phenotypes"
        



        v1=${titre_tbv_to_convert_to_phenotypes}
        v2=${heritability}
        v3=${titre_phenotypes_used}
        v4=${population}

        Rscript ${r_scripts}simulate_phenotypes.R ${v1} ${v2} ${v3} ${v4} 
...
then
    ...
    echo "STEP 1 : prepare real data"

    titre_lines_input=${titre_phenotypes_parents0}
    titre_lines_output=${titre_phenotypes_used}
    ...
    Rscript ${r_scripts}prepare_real_data.R ${v1} ${v2} ${v3} ${v4} ${v5} ${v6} ${v7} ${v8}

fi



if [ ${qtls_info} == "ESTIMATED" ] && [ ${genomic} == "GBLUP" ]
then
