...
job2_1=$(sbatch -o ${job_out} -J ${job_name} --parsable ${r_scripts}gblup_1_all_ldak.sh ${base} ${simulation})
...
job_out=${r_log_value_crosses_gblup}gblup_1_sim${simulation}.out
job_name=gblup${simulation}
job2_1=$(sbatch -o ${job_out} -J ${job_name} --dependency=afterok:${job2_2}:${job2_1} --parsable ${r_scripts}gblup_1_all_ldak.sh ${base} ${simulation})
echo "${job_out} =" >> ${file_jobs}
