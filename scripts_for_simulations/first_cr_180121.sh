...
base=/work/adanguy/these/croisements/scripts/base_cr_050221.sh
...
job3=$(sbatch -o ${job_out} -J ${job_name} --mem=50G --parsable ${r_scripts}calcul.sh ${base} ${nbcores} ${cM} ${simulation})

################################# Etape 3 : training on real data


job_out=${r_log}best_crosses.out
job_name=best
...
job_out=${r_log}sd_predictions.out
job_name=sd
# job5=$(sbatch -o ${job_out} -J ${job_name} --mem=50G --dependency=afterok:${job3} --parsable ${r_scripts}sd_predictions.sh ${base} ${nbcores})
job5=$(sbatch -o ${job_out} -J ${job_name} --mem=100G --parsable ${r_scripts}sd_predictions.sh ${base})
