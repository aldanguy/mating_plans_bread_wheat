...
k=0
output_markers=${r_value_crosses}markers_estimated_all_ldak.txt
output_lines=${r_value_crosses}lines_estimated_all_ldak.txt
...
if [ ${simulation} == "FALSE" ]
    ...
    echo "${r_prepare}" > ${folder_list_gblup}
        ...
        for method in ${genomic[*]}
            ...
            job_name=${typeshort}
            job=$(sbatch -o ${job_out} -J ${job_name} --time=00:20:00 --parsable ${r_scripts}gblup_2_all_ldak.sh ${base} ${simulation} ${method} ${type})
