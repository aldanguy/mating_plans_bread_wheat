...
base=/work/adanguy/these/croisements/scripts/base_cr_100221.sh
...
job4=$(sbatch -o ${job_out} -J ${job_name} --parsable ${r_scripts}best_crosses.sh ${base} ${nbcores} ${generation} ${affixe})
...
population=EA
job_out=${r_log_sd_predictions}sd_predictions_WE_EA.out
job_name=sd_WEEA
# job5=$(sbatch -o ${job_out} -J ${job_name} -c ${nbcores} --mem-per-cpu=10G --dependency=afterok:${job3} --parsable ${r_scripts}sd_predictions.sh ${base} ${nbcores} ${population})
