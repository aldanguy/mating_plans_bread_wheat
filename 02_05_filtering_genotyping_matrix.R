...
# 
#  titre_genotyping_input <-  "/work/adanguy/these/croisements/amont/matrix_nonimput_withoutOTV_names.txt" 
#  titre_lines_input <- "/work2/genphyse/dynagen/adanguy/croisements/160621/prepare/lines_2.txt"     
#  keep_all <- "FALSE"                                                                      
#  titre_genotyping_output <- "/work2/genphyse/dynagen/adanguy/croisements/160621/prepare/genotyping_1.txt"
#  titre_lines_output <- "/work2/genphyse/dynagen/adanguy/croisements/160621/prepare/lines.txt"       
...
while(nrow(snp_pas_ok) >=1 | nrow(lines_pas_ok)>=1){
  ...
  genotyping2 <- genotyping2 %>% dplyr::select(one_of(c("ID", snp_to_keep)))
...
genotyping2bis <- genotyping2 %>% dplyr::select(c(1,which(dom$good==T)+1))
genotyping2ter <- genotyping2%>% dplyr::select(c(which(dom$good==F)+1))
...
#   mutate(used_as_parent= ID %in% genotyping2$ID) %>% 
#   filter(used_as_parent==T) %>% 
#   inner_join(ajd %>% dplyr::select(ID, value), by="ID") %>%
#   arrange(ID) %>%
#   mutate(type="pheno") %>%
#   mutate(sim="FALSE") %>%
#   mutate(qtls=NA) %>%
